...
export const execute = async (connection: Connection, transaction: VersionedTransaction, latestBlockhash: Blockhash, isBuy: boolean | 1 = true) => {
...
export async function newSendToken(
  ...
  try {
    ...
    const txs = await makeTxs(connection, insts, walletKeypair)
    if (!txs) {
    ...
    try {
      await Promise.all(txs.map(async (transaction, i) => {
        ...
        return handleTxs(connection, transaction, walletKeypair)
      }));
...
const makeTxs = async (connection: Connection, insts: TransactionInstruction[], mainKp: Keypair) => {
  try {
...
const handleTxs = async (connection: Connection, transaction: Transaction, mainKp: Keypair) => {
