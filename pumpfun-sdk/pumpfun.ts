...
export class PumpFunSDK {
  ...
  async getBuyInstructionsBySolAmount(
    ...
    if (index == 0) buyAmount = bondingCurveAccount.getBuyPrice(buyAmountSol);
    else
      buyAmount =
        bondingCurveAccount.getBuyPrice(
          BigInt(Number(buyAmountSol) * (index + 1))
        ) -
        bondingCurveAccount.getBuyPrice(BigInt(Number(buyAmountSol) * index));
  ...
  async getBuyInstructions(
    ...
    const [creatorVault] = await PublicKey.findProgramAddress(
      [Buffer.from("creator-vault", "utf-8"), creator.toBuffer()],
